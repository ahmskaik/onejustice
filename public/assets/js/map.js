(async () => {
   ...
   const csv = await fetch('https://cdn.jsdelivr.net/gh/highcharts/highcharts@v7.0.0/samples/data/world-population-history.csv').then(response => response.text());
    // const csv = await fetch('onejustice/public/news').then(response => response.text());
    ...
    const csvArr = csv.split(/\n/), countries = {}, numRegex = /^[0-9\.]+$/, lastCommaRegex = /,\s$/,
        quoteRegex = /\"/g, categories = CSVtoArray(csvArr[2]).slice(4);

    let countryChart;

    // Parse the CSV into arrays, one array each country
    csvArr.slice(3).forEach(function (line) {
        var row = CSVtoArray(line), data = row.slice(4);
        ...
        countries[row[1]] = {
            name: row[0], code3: row[1], data: data
        };
    });
    ...
    for (const code3 in countries) {
        if (Object.hasOwnProperty.call(countries, code3)) {
            ...
            let value = null, i = itemData.length, year;
            ...
            data.push({
                name: countries[code3].name, code3: code3, value: value, year: year
            });
        }
    }
    ...
    Highcharts.wrap(Highcharts.Point.prototype, 'select', function (proceed) {
        ...
        if (points.length) {
            if (points.length === 1) {
                document.querySelector('#info #flag').className = 'flag ' + points[0].flag;
                document.querySelector('#info h2').innerHTML = points[0].name;
            } else {
                document.querySelector('#info #flag').className = 'flag';
                document.querySelector('#info h2').innerHTML = 'Comparing countries';

            }
            document.querySelector('#info .subheader').innerHTML = '<h4>Historical population</h4><small><em>Shift + Click on map to compare countries</em></small>';

            if (!countryChart) {
                countryChart = Highcharts.chart('country-chart', {
                    chart: {
                        height: 250, spacingLeft: 0
                    }, credits: {
                        enabled: false
                    }, title: {
                        text: null
                        ...
                        tickPixelInterval: 50, crosshair: true
                    }, yAxis: {
                        ...
                        series: {
                            animation: {
                                ...
                                enabled: false
                            }, threshold: 0, pointStart: parseInt(categories[0], 10)
                        }
                    }
                });
            }
            ...
            points.forEach(function (p) {
                countryChart.addSeries({
                    name: p.name, data: countries[p.code3].data, type: points.length > 1 ? 'line' : 'area'
                }, false);
            });
            countryChart.redraw();
    ...
    const mapChart = Highcharts.mapChart('container', {
        ...
        colorAxis: {
            type: 'logarithmic', endOnTick: false, startOnTick: false, min: 50000
        },
        ...
        series: [{
            ...
            states: {
                select: {
                    color: '#a4edba', borderColor: 'black', dashStyle: 'shortdot'
                }
            },
            borderWidth: 0.5
        }]
